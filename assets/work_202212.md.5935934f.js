...
<span class="line"></span></code></pre></div><h2 id="_1210" tabindex="-1">1210 <a class="header-anchor" href="#_1210" aria-hidden="true">#</a></h2><ul><li>Nodejs 中的 cluster 模式和 fork 模式区别</li><li>docker 部署前端 nodejs 项目</li><li>注重身体健康和眼睛健康(上了年龄没办法，比不上小年轻了)</li></ul><h2 id="_1211" tabindex="-1">1211 <a class="header-anchor" href="#_1211" aria-hidden="true">#</a></h2><ul><li><strong>docker-compose</strong> 学习</li><li><strong>docerk</strong> 高效部署</li></ul><h2 id="_1212" tabindex="-1">1212 <a class="header-anchor" href="#_1212" aria-hidden="true">#</a></h2><ul><li><strong>docker-compose</strong> 配置学习</li><li>nginx 配置问题 现象：<a href="http://www.bythewayer.com/learn/react" target="_blank" rel="noreferrer">访问</a>网站，刷新之后显示 nginx 页面，nginx.conf 增加配置问题解决</li></ul><p>目前 nginx 配置</p><div class="language-bash"><button title="Copy Code" class="copy"></button><span class="lang">bash</span><pre class="shiki"><code><span class="line"><span style="color:#A6ACCD;">  location  /learn </span><span style="color:#89DDFF;">{</span></span>
...
<span class="line"></span></code></pre></div><h2 id="_1213" tabindex="-1">1213 <a class="header-anchor" href="#_1213" aria-hidden="true">#</a></h2><ul><li><p>将静态资源推送至 OSS(Object Storage Service)云存储服务,下个月搞一搞</p></li><li><p>对于不确定的命令行或者容易忘记的命令行，阔以采用 <strong>docker image --help docker container --h</strong> 等方式强化记忆！！</p></li><li><p><strong>docker-compse.yml</strong> Compose 定位是 「<strong>定义和运行多个 Docker 容器的应用</strong>（Defining and running multi-container Docker applications）」，其前身是开源项目 Fig。</p></li></ul><p>我们知道使用一个 Dockerfile 模板文件可以定义一个单独的应用容器，如果需要定义多个容器就需要服务编排。服务编排有很多种技术方案，今天给大家介绍 Docker 官方产品 Docker Compose。 Dockerfile 可以让用户管理一个单独的应用容器；而 Compose 则允许用户在一个模板(YAML 格式) 中定义一组相关联的应用容器( 被称为一个 project，即项目)，例如一个 Web 服务容器再加上后端的数据库服务容器等。</p><p>Compose 中有两个重要的概念：</p><ul><li>服务 (service)：一个应用的容器，实际上可以包括若干运行<strong>多个镜像的容器实例</strong>。</li><li>项目 (project)：由一组关联的应用容器组成的一个完整业务单元，在 docker-compose.yml 文件中定义。 Compose 的默认管理对象是项目，通过子命令对项目中的一组容器进行便捷地生命周期管理。</li></ul><div class="language-bash"><button title="Copy Code" class="copy"></button><span class="lang">bash</span><pre class="shiki"><code><span class="line"><span style="color:#A6ACCD;">version: </span><span style="color:#89DDFF;">&#39;</span><span style="color:#C3E88D;">3</span><span style="color:#89DDFF;">&#39;</span></span>
...
<span class="line"></span></code></pre></div><p>启动项目脚本</p><div class="language-bash"><button title="Copy Code" class="copy"></button><span class="lang">bash</span><pre class="shiki"><code><span class="line"><span style="color:#676E95;"># --build: 每次启动容器前构建镜像</span></span>
<span class="line"><span style="color:#A6ACCD;">docker-compose up -d --build</span></span>
<span class="line"></span></code></pre></div><ul><li>越看重的东西越比较在意，反正现在就这样了，做做表面功夫，12 月 21 日年终述职</li><li><strong>--chown=nextjs:nodejs</strong></li></ul><div class="language-bash"><button title="Copy Code" class="copy"></button><span class="lang">bash</span><pre class="shiki"><code><span class="line"></span>
<span class="line"><span style="color:#A6ACCD;">- COPY --from=builder --chown=nextjs:nodejs /app/.next ./.next 这句话中的 --chown=nextjs:nodejs 代表啥</span></span>
<span class="line"></span>
<span class="line"><span style="color:#A6ACCD;">在使用该指令的时候还可以加上 </span><span style="color:#89DDFF;">**</span><span style="color:#A6ACCD;">--chown=</span><span style="color:#89DDFF;">&lt;</span><span style="color:#A6ACCD;">user</span><span style="color:#89DDFF;">&gt;</span><span style="color:#A6ACCD;">:</span><span style="color:#89DDFF;">&lt;</span><span style="color:#A6ACCD;">group</span><span style="color:#89DDFF;">&gt;**</span><span style="color:#A6ACCD;"> 选项来改变文件的所属用户及所属组。</span></span>
<span class="line"></span></code></pre></div><h3 id="_1214" tabindex="-1">1214 <a class="header-anchor" href="#_1214" aria-hidden="true">#</a></h3><ul><li>宝贝考研考生定在了上海浦东新区建桥学院，需要提前安排好饮食和交通</li><li>今天需要把快应用剩下部分完成，明天开始做自分享</li></ul><h2 id="_1215" tabindex="-1">1215 <a class="header-anchor" href="#_1215" aria-hidden="true">#</a></h2><ul><li>疫情越来越严重，没有早点囤药，失策，一定要在考试之前确保不能阳</li><li>搞了一天的快应用，明天还要发版本，有点赶</li></ul><h2 id="_1216" tabindex="-1">1216 <a class="header-anchor" href="#_1216" aria-hidden="true">#</a></h2><ul><li>每次听到有多少爱阔以重来，想到一位长辈，如今长辈已经不在了，甚是怀念，所以在有生之年让自己不能留有遗憾</li></ul><h2 id="_1217" tabindex="-1">1217 <a class="header-anchor" href="#_1217" aria-hidden="true">#</a></h2><ul><li>周末在家躺平，把 oppo 快应用和自分享的需求做了下</li><li>准备年终述职和绩效自评</li></ul><h2 id="_1218" tabindex="-1">1218 <a class="header-anchor" href="#_1218" aria-hidden="true">#</a></h2><ul><li>宝贝 21:30 温度 39.0 23:30 温度 38.5</li><li>1.助力列表 2.付费提示 3.接口测试 4.埋点 5.oppo</li></ul><h2 id="_1219" tabindex="-1">1219 <a class="header-anchor" href="#_1219" aria-hidden="true">#</a></h2><ul><li>忙碌的周一，1.助力列表 2.付费提示 3.接口测试 4.埋点 5.oppo 全部搞完</li><li>年终述职报告</li><li>自分享付费样式和实验</li></ul>`,66),o=[e];function c(t,r,i,D,y,C){return a(),n("div",null,o)}const d=s(p,[["render",c]]);export{F as __pageData,d as default};
