...
- github 中的 [actions 脚本 cross-the-world/ssh-scp-ssh-pipelines ](https://github.com/cross-the-world/ssh-scp-ssh-pipelines)如何执行两个命令
...
- 1507
- 手写 quick sort
- [手写 extends](https://bigfrontend.dev/zh/problem/write-your-own-extends-in-es5)
...
- 1510
- node-sass 安装问题，需要安装 python2 环境，遇到路径找不到问题，如何解决

:::info

如何将本地的安装路径换成 /usr/local/opt/python@2/bin/python2.7

- which python  
  usr/local/bin/python2.7

- 第一步 创建缺失的目录结构
  sudo mkdir -p /usr/local/opt/python@2/bin

- 第二步创建软连接
  sudo ln -s /usr/local/bin/python2.7 /usr/local/opt/python@2/bin/python2.7

- 第三步 本地验证
  /usr/local/opt/python@2/bin/python2.7 --version
...
- 1513
- 昨天台风靠近上海，3 点下班，路过陈记烤鸭店，给媳妇带了柠檬无辜鸡爪和五香猪蹄，此时此刻的媳妇高兴坏了，幸福就是这种小事
- 前后端服务迁移，如何加快进度？每天都是在做重复工作，有点无聊
